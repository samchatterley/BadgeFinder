...
exports.registerSecondaryUserSchema = Joi.object({
    ...
    earned_badges: Joi.array().items(Joi.string()).required().error(new InvalidEarnedBadgesError()),
    required_badges: Joi.array().items(Joi.string()).required().error(new InvalidRequiredBadgesError()),
});
...
exports.addBadgeSchema = Joi.object({
    userId: Joi.string().length(24).regex(/^[0-9a-fA-F]{24}$/).required().error(new UserNotFoundError()),
    badgeId: Joi.string().length(24).regex(/^[0-9a-fA-F]{24}$/).required().error(new BadgeNotFoundError()),
});
...
exports.updateBadgeRequirementSchema = Joi.object({
    badgeId: Joi.string().length(24).regex(/^[0-9a-fA-F]{24}$/).required().error(new BadgeNotFoundError()),
    requirementId: Joi.string().length(24).regex(/^[0-9a-fA-F]{24}$/).required().error(new RequirementNotFoundError()),
});