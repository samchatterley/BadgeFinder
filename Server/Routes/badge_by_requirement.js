...
router.get('/', async (req, res) => {
  try {
    ...
    const matchingRequirements = await requirements
      .find({
        requirement_string: { $regex: `.*${query}.*`, $options: 'i' }
      })
      .toArray()

    const matchingBadgeIds = new Set(
      matchingRequirements.map((r) => r.badge_id)
    )

    const matchingBadges = await badges
      .find({ ID: { $in: Array.from(matchingBadgeIds) } })
      .toArray()
