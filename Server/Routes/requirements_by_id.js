...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const result = await collection.find({badge_id: parseInt(badgeId, 10)}).toArray();
    logger.info(`${result.length} requirements found for badge ID: ${badgeId}`, result);
    res.send(result);
  } catch (err) {
    logger.error(`Error occurred while fetching requirements for badge ID: ${badgeId}`, err);
