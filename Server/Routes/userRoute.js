...
module.exports = (userInstance) => {
  ...
  router.post(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      try {
        ...
        if (!badgeId) {
          throw new UserErrors.InvalidBadgeIdError('BadgeId is required')
        }
        ...
        const updatedUser = await userInstance.addBadge(
          new ObjectId(userId),
          new ObjectId(badgeId)
        )
    ...
    asyncHandler(async (req, res, next) => {
      ...
      try {
        ...
        if (!user?.earned_badges.find((badge) => badge.badge_id === badgeId)) {
          throw new UserErrors.DoesNotHaveBadgeError(
            'User does not have the badge'
          )
        }
        const updatedUser = await userInstance.removeBadge(
          new ObjectId(userId),
          new ObjectId(badgeId)
        )
        logger.info('User updated successfully')
   ...
   */
  router.patch(
    '/:id/badge/:badgeId/requirement/:requirementId',
    authenticate,
    asyncHandler(async (req, res, next) => {
      const userId = req.params.id
      const badgeId = req.params.badgeId
      ...
      try {
        logger.info(
          `Received PATCH request for user with id ${userId}, badge id ${badgeId}, and requirement id ${requirementId}`
        )
        if (completed === undefined) {
          throw new UserErrors.InvalidCompletionStatusError(
            'Completed is required'
          )
        }
        const user = await userInstance.findById(new ObjectId(userId))
        if (!user) throw new UserErrors.UserNotFoundError('User not found')
        const badge = user.earned_badges.find(
          (badgeItem) => badgeItem.badge_id === badgeId
        )
        if (!badge) {
          throw new UserErrors.BadgeNotFoundError(
            `Badge with id ${badgeId} not found`
          )
        }
        if (!badge.requirements) {
          throw new UserErrors.UserError(
            'Badge does not have any requirements'
          )
        }
        const requirement = badge.requirements.find(
          (requirementItem) => requirementItem.requirement_id === requirementId
        )
        if (!requirement) {
          throw new UserErrors.RequirementNotFoundError(
            `Requirement with id ${requirementId} not found`
          )
        }
        const updateOperation = {
          $set: {
            'earned_badges.$[badge].requirements.$[requirement].completed':
              completed
          }
        }
        const options = {
